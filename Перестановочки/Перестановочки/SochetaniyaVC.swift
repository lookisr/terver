...
class SochetaniyaVC: UIViewController {
    ...
    override func viewDidLoad() {
        super.viewDidLoad()
        onOff.addTarget(self, action: #selector(SochetaniyaVC.indexChanged(_:)), for: .valueChanged)
    ...
    @IBAction func buttonTapped(_ sender: UIButton) {
        if flag{
            ...
            print(Int(n.text!)!+Int(n1.text!)! - 1)
            let answ = factorial(Int(n.text!)!+Int(n1.text!)! - 1) / ((Double(Int(n.text!)!) - 1) * factorial(Int(n1.text!)!))
            ...
            print(Int(n.text!)!+Int(n1.text!)! - 1)
            let answ = factorial(Int(n.text!)!) /  ((factorial(Int(n.text!)!-Int(n1.text!)!)) * factorial(Int(n1.text!)!))
            answer.text = NSDecimalNumber(decimal: Decimal(answ)).stringValue
